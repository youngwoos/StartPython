...
```{python}
# 순서 구하기
class_order = mpg.groupby('drv') \
                 .agg(median_hwy = ('hwy', 'median')) \
                 .sort_values('median_hwy', ascending = False)
class_order
...
```{python eval = F}
# 상자 그림에 적용
sns.boxplot(data = mpg, x = 'drv', y = 'hwy', order = class_order.index)
```

```{python echo = F, out.width = '80%'}
p = sns.boxplot(data = mpg, x = 'drv', y = 'hwy', order = class_order.index)
plt.show()
```
...
```{python eval = F}
# 박스플롯 정렬하기
sns.boxplot(data = df, x = 'class', y = 'cty', order = ['compact', 'subcompact', 'suv'])
```

```{python echo = F, out.width = '80%'}
p = sns.boxplot(data = df, x = 'class', y = 'cty', order = ['compact', 'subcompact', 'suv'])
plt.show()
```
...
```{python}
# 순서 구하기
class_order = df.groupby('class') \
                .agg(median_cty = ('cty', 'median')) \
                .sort_values('median_cty', ascending = False) \
                .reset_index()
...
```{python eval = F}
# 박스플롯 정렬하기
sns.boxplot(data = df, x = 'class', y = 'cty', order = class_order['class'])
```

```{python echo = F, out.width = '80%'}
# 박스플롯 정렬하기
p = sns.boxplot(data = df, x = 'class', y = 'cty', order = class_order['class'])
plt.show()
```
